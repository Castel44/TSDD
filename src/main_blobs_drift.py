...
def plot_embedding(train, test, Y_train, Y_test, centroids, classes, saver, figname='train_embedding'):
    ...
    plt.tight_layout()
    plt.gcf().savefig('/home/castel/PycharmProjects/torchembedding/results/CD_results/embedding/no_cstr.pdf', bbox_inches='tight')
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='MovingRBF')

    parser.add_argument('--drift_type', type=str, default='step', choices=['gradual', 'step'])
    ...
    parser.add_argument('--init_seed', type=int, default=999, help='RNG seed. Typ. 42, 420, 1337, 0, 69.')
...
def single_experiment(args, path):
    ...
    X, y, centers = make_blobs(n_samples=2000, n_features=n_features, centers=classes, cluster_std=1.0,
                               return_centers=True)
    x_test_, Y_test_, centers_end = make_blobs(n_samples=1000, n_features=n_features, centers=classes, cluster_std=1.0,
                                               return_centers=True)
    Y_test_ = Y_test_[:, np.newaxis]
    x_test, Y_test = make_blobs(n_samples=500, n_features=n_features, centers=centers, cluster_std=1.0)
    Y_test = Y_test[:, np.newaxis]
    if args.drift_type == 'gradual':
        for i in [x * 0.1 for x in range(2, 12, 2)]:
            x_, y_ = make_blobs(n_samples=100, n_features=n_features, centers=linear_interp(centers_end, centers, i),
                                cluster_std=1.0)
            y_ = y_[:, np.newaxis]
    ...
    unique_test = np.setdiff1d(np.unique(Y_test), unique_train)
    ...
    df['drift_type'] = args.drift_type
    df['classes'] = args.classes
